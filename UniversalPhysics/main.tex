\documentclass[b5paper,opensource,sourcefont,parskip,color]{qyxf-book}
...
\solve
$F=\int I\di{l}\cdot B$。在这里可以将该电流投影到垂直于$B$的方向上，等效为从a指向圆心的电流。投影长度为$R$，故安培力大小为$BIR$，方向用等效的电流判断即可。
...
\exercise{15}$5.36\times 10^{-3}\textrm{kg}\cdot m^2/\textrm{s}^2$
...
\begin{align*}
	M&=Fa\sin \angle{40}\\
	&=BIa^2\sin \angle{40}\\
	&=7.2\sin \angle{40}\times 10^{-3}=5.36\times 10^{-3}
\end{align*}
保持位置不变，需要一等大反向的力矩。
...
该系统可看作半径为R、电流密度为i的无限长圆柱面与半径为r、电流密度为-i的无限长圆柱面的组合。
